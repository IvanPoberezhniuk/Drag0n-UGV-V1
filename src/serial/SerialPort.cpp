...
bool SerialPort::open(const std::string& portName, uint32_t baud) {
    ...
    dcb.fOutX           = FALSE;
    dcb.fInX            = FALSE;
    dcb.fDtrControl     = DTR_CONTROL_DISABLE;  // don't reset module on open
    dcb.fRtsControl     = RTS_CONTROL_DISABLE;

    if (!SetCommState(h, &dcb)) {
        spdlog::error("SerialPort: SetCommState failed (error {}). "
                      "If baud {} is unsupported, try 400000 or 500000.", GetLastError(), baud);
        CloseHandle(h);
        return false;
    }
    ...
    timeouts.WriteTotalTimeoutConstant   = 50;  // 50ms write timeout — safe disconnect guard
...
std::string SerialPort::autoDetect() {
    // USB VID:PID strings for chips commonly used in ExpressLRS TX modules
    static const char* knownIds[] = {
        "VID_10C4&PID_EA60",  // Silicon Labs CP210x — most common in ELRS modules
        "VID_1A86&PID_7523",  // CH340
        "VID_1A86&PID_7522",  // CH341
        "VID_0483&PID_5740",  // STM32 Virtual COM Port (newer ELRS firmware)
    ...
    for (DWORD i = 0; SetupDiEnumDeviceInfo(devInfo, i, &devData); ++i) {
        ...
        // Get the COMx name from the device's registry key
        HKEY hKey = SetupDiOpenDevRegKey(
            devInfo, &devData, DICS_FLAG_GLOBAL, 0, DIREG_DEV, KEY_READ);
